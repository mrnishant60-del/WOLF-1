...
class BaseScrim(BaseModel):
    ...
    async def validate_perms(self, bot: Wolf) -> Tuple[bool, Union[bool, str]]:
        ...
        if not reg_channel:
            return False, "Wolf can't see your registration channel. Give Perms."

        _p = reg_channel.permissions_for(guild.me)
        if not all((_p.manage_channels, _p.manage_permissions, _p.manage_messages)):
            return False, "Wolf can't manage this registration channel. Give Perms."

        role = guild.get_role(self.role_id)
        if not role:
            return False, "Wolf couldn't find your sucess role."

        if role >= guild.me.top_role:
            return False, "Drag Wolf role above your success role."
    ...
    async def create_scrim(self, bot: Wolf):
        if not await bot.is_premium_user_or_guild(self.host_id, self.guild_id):
            if await Scrim.filter(guild_id=self.guild_id).count() >= 3:
                return False, "Cannot create more than 3 scrims without Premium. Either you or your server needs Wolf Premium."
    ...
    async def update_scrim(self, bot: Wolf):
    ...
    async def __check_bot_perms(self, bot: Wolf):
        ...
        if not all((g_perms.manage_channels, g_perms.manage_roles, g_perms.manage_messages)):
            return False, "Wolf needs manage channels, manage roles permission."

        if not all((g_perms.add_reactions, g_perms.embed_links)):
            return False, "Wolf needs add reacions & embed links permission."
