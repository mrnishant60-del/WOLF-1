...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Cog, List[commands.Command]]):
        ...
        for cog, cmds in mapping.items():
            if cog and cog.qualified_name not in hidden and cog.qualified_name != "Dev" and await self.filter_commands(cmds, sort=True):
                embed.add_field(
                    inline=False,
        ...
        if ctx.author.id in ctx.config.DEVS:
            dev_cog = ctx.bot.get_cog("Dev")
            if dev_cog:
                admin_cmds = []
                for cmd in dev_cog.get_commands():
                    if isinstance(cmd, commands.Group):
                        for subcmd in cmd.walk_commands():
                            admin_cmds.append(f"`{subcmd.qualified_name}`")
                    else:
