...
async def check_scrim_requirements(bot, message: discord.Message, scrim: Scrim) -> bool:
    ...
    valid_logo = any(att.content_type and att.content_type.startswith('image/') for att in message.attachments)
    if not valid_logo:
        _bool = False
        bot.dispatch("scrim_registration_deny", message, constants.RegDeny.nologo, scrim)
