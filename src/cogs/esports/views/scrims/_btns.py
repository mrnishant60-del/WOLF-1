    ...
    async def callback(self, interaction: Interaction):
        ...
        if not await self.ctx.is_premium_user_or_guild():
            return await self.ctx.error(
                "**This feature requires either you or your server to have Wolf Premium.**\n\n`Purchase Wolf Premium to use this feature.`", 5
            )
...
class DuplicateTags(ScrimsButton):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not await self.ctx.is_premium_user_or_guild():
            return await self.ctx.error(
                "This feature requires either you or your server to have Wolf Premium.", 4
            )
...
class SlotsUpdateChannel(ScrimsButton):
    def __init__(self, ctx: Context, letter: str):
        super().__init__(emoji=ri(letter))
        self.ctx = ctx
    ...
    async def callback(self, interaction: Interaction):
        ...
        if self.view.record.slots_update_channel_id:
            self.view.record.slots_update_channel_id = None
            await self.view.refresh_view()
            return await self.ctx.success("Removed slots update channel, click again to set.", 5)
        
        m = await self.ctx.simple("Mention the channel where you want me to send remaining slots updates.")
        channel = await inputs.channel_input(self.ctx, delete_after=True)
        await self.ctx.safe_delete(m)

        self.view.record.slots_update_channel_id = channel.id

        await self.view.refresh_view()


class SlotsReminderThresholds(ScrimsButton):
    def __init__(self, ctx: Context, letter: str):
        super().__init__(emoji=ri(letter))
        self.ctx = ctx
    ...
    async def callback(self, interaction: Interaction):
        ...
        m = await self.ctx.simple(
            "Enter the slot numbers when you want reminders to be sent.\n"
            "**Example:** `20,10,5` will send reminders at 20, 10, and 5 slots remaining.\n"
            "Enter numbers separated by commas (Max 5 thresholds, each max `30`)"
        )
        thresholds_input = await inputs.string_input(self.ctx, delete_after=True)
        await self.ctx.safe_delete(m)
        
        try:
            thresholds = [int(x.strip()) for x in thresholds_input.split(",")]
            
            if len(thresholds) > 5:
                return await self.ctx.error("Maximum 5 reminder thresholds allowed.", 5)
            
            if any(t < 1 or t > 30 for t in thresholds):
                return await self.ctx.error("Each threshold must be between 1 and 30.", 5)
            
            thresholds = sorted(set(thresholds), reverse=True)
            self.view.record.slots_reminder_thresholds = thresholds
            
        except ValueError:
            return await self.ctx.error("Invalid format. Please enter numbers separated by commas.", 5)

        await self.view.refresh_view()


class SlotsReminderRole(ScrimsButton):
    def __init__(self, ctx: Context, letter: str):
        super().__init__(emoji=ri(letter))
        self.ctx = ctx
    ...
    async def callback(self, interaction: Interaction):
        ...
        if self.view.record.slots_reminder_role_id:
            self.view.record.slots_reminder_role_id = None
            await self.view.refresh_view()
            return await self.ctx.success("Removed slots reminder role, click again to set.", 5)
        
        m = await self.ctx.simple("Mention the role you want to ping when slots reminder is sent.")
        role = await inputs.role_input(self.ctx, delete_after=True)
        await self.ctx.safe_delete(m)
        
        self.view.record.slots_reminder_role_id = role.id
