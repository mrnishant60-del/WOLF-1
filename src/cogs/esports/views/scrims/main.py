...
class ScrimsMain(ScrimsView):
    ...
    async def initial_embed(self):
        _e = discord.Embed(color=0x00FFB3, title="Wolf's Smart Scrims Manager", url=self.ctx.config.SERVER_LINK)
    ...
    async def create_new_scrim(self, interaction: Interaction, button: ui.Button):
        ...
        if not await self.ctx.is_premium_user_or_guild():
            if await Scrim.filter(guild_id=self.ctx.guild.id).count() >= 3:
                return await self.ctx.premium_mango("Only 3 scrims can be created with free plan. Either you or your server needs Wolf Premium.")
