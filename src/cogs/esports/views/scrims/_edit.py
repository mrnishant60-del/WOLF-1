...
class ScrimsEditor(ScrimsView):
    ...
    async def initial_message(self):
        ...
        fields = {
            ...
            f"Slots Update Channel 💎": getattr(scrim.slots_update_channel, "mention", "`Not-Set`"),
            f"Slots Reminder Thresholds 💎": ", ".join([f"`{t}`" for t in scrim.slots_reminder_thresholds]) if scrim.slots_reminder_thresholds else "`Not-Set`",
            f"Slots Reminder Role 💎": getattr(scrim.slots_reminder_role, "mention", "`Not-Set`"),
        }
    ...
    async def _add_buttons(self):
        ...
        self.add_item(SlotsUpdateChannel(self.ctx, "u"))
        self.add_item(SlotsReminderThresholds(self.ctx, "v"))
        self.add_item(SlotsReminderRole(self.ctx, "w"))
