...
def is_premium_user_or_guild():
    async def predicate(ctx: Context):
        user_check = await User.get_or_none(user_id=ctx.author.id)
        guild_check = await Guild.get_or_none(guild_id=ctx.guild.id)
        
        is_premium_user = user_check and user_check.is_premium
        is_premium_guild = guild_check and guild_check.is_premium
        
        if not is_premium_user and not is_premium_guild:
            raise NotPremiumUserOrGuild()
